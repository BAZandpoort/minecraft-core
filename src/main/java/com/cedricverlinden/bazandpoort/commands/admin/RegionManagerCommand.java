    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String label, @NotNull String[] args) {
        ...
        if (args[0].equals("list")) {
            if (regions == null || regions.getRegions() == null || regions.getRegions().isEmpty()) {
                player.sendMessage(ChatUtil.color("There are no available regions."));
                return true;
            }

            player.sendMessage(ChatUtil.color("&8&m----------------------------------------"));
            player.sendMessage(ChatUtil.color("&2Available regions:"));
            ...
            player.sendMessage("");
            player.sendMessage(ChatUtil.color("&2Added regions:"));
            ...
            for (String name : lectures) {
                ...
                if (regionName == null || regionName.isEmpty()) {
                    player.sendMessage(ChatUtil.color("&8- &c" + name + " (none)"));
                    continue;
                }

                ProtectedRegion region = regions.getRegion(regionName);
                if (region == null) {
                    player.sendMessage(ChatUtil.color("&8- &c" + name + " (not valid)"));
                    continue;
                }
        ...
        if (args[0].equals("add")) {
            ...
            if (lecturesSection == null) {
                player.sendMessage(ChatUtil.color("&cThere are no available lectures."));
                return true;
            }

            ProtectedRegion region = regions.getRegion(regionName);
            if (region == null) {
                player.sendMessage(ChatUtil.color("&cThe region '" + regionName + "' does not exist."));
                return true;
            }
...
//            }
            ...
            player.sendMessage(ChatUtil.color("&aSuccessfully added '&2" + regionName + "&a' as region to the &2" + lectureName + " &alecture."));
            return true;
        }
